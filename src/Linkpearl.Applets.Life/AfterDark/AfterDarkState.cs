...
internal sealed class AfterDarkState
{
    ...
    public bool FeedEveryone { get; set; } = true;
    ...
    public static AfterDarkState Load(HostPaths paths, string fallbackName)
    {
        var state = new AfterDarkState();
        var path = paths.State("afterdark.json");
        var fromDisk = false;
        if (File.Exists(path))
        {
            try
    ...
    public void TickHold(bool overMark, bool held, float delta)
    {
        ...
        {
            HoldMark += delta;
            if (HoldMark >= AfterDarkChrome.HoldSeconds)
            {
                StartWash(!Night);
            }
        }
    ...
    public void TickWash(float delta, HostPaths paths)
    {
        ...
        Wash += delta;
        if (Wash < AfterDarkChrome.WashSeconds)
        {
            return;
        }
    ...
    public void Bind(PearlSnapshot snap)
    {
        ...
        foreach (var person in snap.People)
        {
            ...
            Roster.Add(new ScenePerson(id, person.Id, person.DisplayName.Length > 0 ? person.DisplayName : "Someone",
                handle, person.PhoneNumber, string.Empty, person.IsMutual, 0, false, WashOf(id),
...
internal readonly record struct ScenePerson(
    ...
    Vector4 Wash, string[] Intents, string[] Tags, string AvatarUrl = "");
...
internal static class SceneBook
{
    ...
    {
        "Friends", "GPose", "Collab", "Sharing", "Wandering", "Relationship", "ERP",
    };

    public static readonly string[] DayIntents = { "Friends", "GPose", "Collab", "Sharing", "Wandering" };
    ...
    public static string[] DayHashes { get; } = { "#Crystal", "#FreeCompany", "#GoodVibes", "#Daylight" };
