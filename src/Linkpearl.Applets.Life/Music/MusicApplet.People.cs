...
public sealed partial class MusicApplet
{
    private IReadOnlyList<MusicPerson> Roster() =>
        MusicRoster.Directory(state, pearl.Current, community.Live, community.Mine, community.Broadcasting);
    ...
    private void DrawPersonRow(in AppletFrame frame, Rect row, MusicPerson person)
    {
        ...
        if (!person.Mine)
        {
            ...
            frame.Text.DrawIn(follow, on ? "Followed" : "Follow",
                new TextStyle(FontRole.CaptionStrong, on ? MusicChrome.Ink : MusicChrome.Purple, TextAlign.Center));
