...
/// One-shot: dials the relay, submits a show report, and disconnects - doesn't register as a host or join a
/// room, mirroring BugReportClient exactly.
public static class ShowReportClient
{
    public static async Task<(bool Success, string? Error)> SubmitAsync(SubmitShowReportMessage report)
    {
        try
        {
            ...
            var bytes = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(RelayEnvelope.For(RelayMessageType.SubmitShowReport, report)));
            ...
            if (envelope?.Type != RelayMessageType.ShowReportAck)
                return (false, "Relay sent an unexpected response.");

            var ack = envelope.ReadPayload<ShowReportAckMessage>();
